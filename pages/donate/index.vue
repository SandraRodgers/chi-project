...
<div class="bg-whitesmoke pt-28 lg:pt-40 pb-6 md:pb-8 lg:pb-16 flex flex-col items-center justify-center w-full">
        ...
        <div class="h-screen bg-white w-11/12 flex justify-center pt-10">{{(JSON.parse(test).Title)}}</div>
        <div class="h-screen bg-white w-11/12 flex justify-center pt-10">{{test}}</div>
...
export default {
    ...
    const test = await $http.$get('http://localhost:8888/.netlify/functions/testapi')
    return { test }
